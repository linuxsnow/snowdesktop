...
handle_setup_stage() {
    ...
    if [ "$BAZAAR_TS_TYPE" = install ]; then
        case "$BAZAAR_TS_APPID" in
            com\.visualstudio\.*)
                # since the appid belongs to vscode, we continue
                # with the hook
                echo 'ok'
                ;;
            *)
...
handle_teardown_dialog_stage() {
    ...
    case "$BAZAAR_HOOK_DIALOG_RESPONSE_ID" in
        install-brew)
            # if the user pressed "Install Visual Studio Code via Brew",
            # continue
            echo 'ok'
            ;;
        *)
...
handle_teardown_stage() {
    ...
    # Let's always prevent the user from installing Visual Studio Code
    echo 'deny'
